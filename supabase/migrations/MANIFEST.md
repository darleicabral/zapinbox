...
| `20260726120000` | `0028_contacts_custom_fields` | `contacts.custom_fields` (jsonb, default `{}`, not null) — campos custom POR CONTATO, mesmo modelo de `crm_leads.custom_fields`. Motivação (Itaville pós-venda): a lista de contatos virou tela de trabalho ativa (Empreendimento, "Liguei", Status da abordagem) e esses dados são do contato, não do chamado. A definição dos campos (chave/rótulo/opções) fica na aplicação (`lib/contacts/fields.ts`), então nenhum tenant precisa de coluna nova. Idempotente (`add column if not exists`), portável em psql. |
| `20260802140000` | `0029_visibilidade_por_numero` | Dois números de WhatsApp na mesma org com **parede entre eles**. Motivação (Itaville): Samilis fica no número atual, Júlia entra só no número novo, Gabriela (gerente) vê tudo. Adiciona `contacts.owner_channel_session_id` (número de ORIGEM do contato — contato não tem número por natureza, e o mesmo telefone não pode existir 2x na org) com backfill genérico para orgs de um único número; cria `user_channel_sessions` (atribuição usuário↔número, escrita só de gerente/admin) e as funções `fn_user_session_ids()` / `fn_contact_visible()`. **Substitui** (não acrescenta) as policies de `conversations`, `messages`, `contacts`, `crm_leads` e `channel_sessions` para filtrar por tenant **E** número — policies se somam por OR, então acrescentar não travaria nada. `fn_upsert_wa_contact` ganha 7ª arg `p_session` e marca o dono do contato no primeiro contato (sem reatribuir quem já tem dono). ⚠️ **Regra de compatibilidade:** usuário SEM atribuição vê TODOS os números — sem ela, todo mundo perderia acesso no instante do deploy. Idempotente, portável em psql. |
