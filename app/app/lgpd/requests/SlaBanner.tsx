...
export function SlaBanner({ requests }: SlaBannerProps) {
  ...
  const critical = active.filter((r) => r.sla_bucket === "overdue" || r.sla_bucket === "critical");
    ...
    return (
      ...
      >
        ...
        <span>
          ...
          {critical.length === 1 ? "solicitação crítica" : "solicitações críticas"} — SLA vencido ou
          inferior a 2 dias. Ação imediata requerida.
        </span>
      </div>
    );
  }
    ...
    return (
      ...
      >
        ...
        <span>
          ...
          {warning.length === 1 ? "solicitação em alerta" : "solicitações em alerta"} — mais de 50%
          do prazo consumido.
        </span>
      </div>
    );
